...
def CL_index(txt): 
    ...
    '''
    # L= Lettres ÷ Mots × 100
    L = len(txt.replace(' ','')) / len(txt.split(' ')) * 100
...
def preprocess_text(text):
    """
    Fonction pour nettoyer et prétraiter un texte.
    ...
    # Lemmatisation - Convertir les mots en leur forme de base
    lemmatizer = WordNetLemmatizer()
    tokens = [lemmatizer.lemmatize(word) for word in tokens]

    # Rejoindre les mots prétraités pour former un texte propre
