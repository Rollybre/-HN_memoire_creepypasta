...
%% Modifier cette partie. Arriver ici nous permet de prendre le temps de problématiser. Pour ce qui ets méthdoe on décale ça plus loin.
	...
	%% PROBLÉMATIQUES
	%% DOubles => caractérisation du fond et  de la forme d'un genre hybride, entre codes littéraires et viralité. => explication de la viralité :  Peut-on expliquer le succès de ces histoires à travers leurs caractéristiques textuelles ? 
	%% Un genre fortement codifié : (plus tard) comme on a les règles est il possible de voir comment les histoires à succès se contrsuient par rapport à elles ? Sont-elles à l'origines des règles ou au contraire est-ce que les règles entraînent l'émergence de poit de référence ? 
	...
	\chapter{Exploration et visualisation}
	%Exploration
	...
	Par cette méthode nous avons isolé douze thèmes suivants (cf. figure 4.1) : 
	\begin{itemize}
		\item le sommeil
	...
	Pour ce faire, nous avons suivi la même méthode d'extraction des thèmes cette fois-ci sur un ensemble de description de films d'horreur extraits de la base de données \textit{The Movie Data Base}\footnote{\url{https://www.themoviedb.org/}} (cf. Figure 4.2)	
	...
	Notons aussi que cette caractérisation n'est pas dépendante de la plateforme d'origine : à quelques différences près, les thèmes présents sur les deux plateformes sont les mêmes (cf. Figure 4.3 et Figure 4.4). 
	...
	\chapter{Richesse Lexicale et indice de lisibilité}
...
% Lisibilité : une score faible => une littérature accessible
	...
	\section{Les indices de lisibilités} 
	...
	\section{Les indices de richesse lexicales}
	%Comparaison de la lisibilité  => Plus facilement lisible que de la litt et que le web en moyenne (à nuancer)
	Autres formes de mesure, les indices de richesse lexicale nous permettent d'entrevoir la littérarité à travers le prisme du vocabulaire :
	CCL : pas ultra pertinent les hapax dans des textes courts sont pas top top .
	%Une littérautre qui à tendance à ne pas se littérariser
	
	\chapter{Emotions}

\chapter{Regression Logistiques  }
	%A faire  
	...
	%Les règles
	%Clarifier les questions de recherches : Caractérisation du genre ( formes, sujets...) Différence entre les plateformes. Assez pour idenitifer le succès ? 
	%Mentionner la présence d'image ... et leur disparition
		%pas vraiment d'hypothèse la dessus : peut être associé à la littérarisation du genre ? 
